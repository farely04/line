...
// 2. อ่านค่า Environment Variables (LINE_ACCESS_TOKEN, LINE_SECRET, GEMINI_API_KEY) 
//    ที่ตั้งบน Vercel Dashboard (Settings → Environment Variables)
const LINE_ACCESS_TOKEN = process.env.LINE_ACCESS_TOKEN;
const LINE_SECRET       = process.env.LINE_SECRET;
const GEMINI_API_KEY    = process.env.GEMINI_API_KEY;

// 3. สร้าง LINE SDK Client
const lineClient = new Client({
  channelAccessToken: LINE_ACCESS_TOKEN,
  channelSecret:      LINE_SECRET,
});

// 4. ปิด body parser ของ Vercel เพื่อให้เราดักอ่าน raw body เอง (จำเป็นสำหรับตรวจ signature ของ LINE)
export const config = {
  api: {
    bodyParser: false,
  },
...
// 5. ฟังก์ชันสำหรับเรียกใช้งาน Google Gemini (ข้อความ)
...
// 6. ฟังก์ชันสำหรับเรียกใช้งาน Google Gemini (รูปภาพ)
...
// 7. ฟังก์ชันดาวน์โหลดรูปภาพจาก LINE Server แล้วแปลงเป็น Base64
...
// 8. ฟังก์ชันหลักที่ Vercel จะเรียกเมื่อมี POST มาที่ /api/webhook
export default async function handler(req, res) {
  ...
  // 8.2 อ่าน raw request body (Buffer) เพื่อตรวจ signature ของ LINE
  ...
  // 8.5 ดึง events ที่ LINE ส่งมา (อาจเป็นอาร์เรย์หลายรายการ)
  const events = body.events || [];

  // 8.6 วนลูปประมวลผลแต่ละ event
  const promises = events.map(async (event) => {
    if (event.type !== "message") {
      // กรณีไม่ใช่ message (เช่น follow, unfollow ฯลฯ) ให้ข้าม
      return Promise.resolve();
    }

    // 8.6.1 ถ้าเป็นข้อความ text
    ...
    // 8.6.3 กรณีอื่นๆ (video, sticker ฯลฯ) ไม่ต้องทำอะไรต่อ
    return Promise.resolve();
  });
